...
service.interceptors.request.use(config => {
  ...
  let myconfig = {baseURL: config.baseURL, url : config.url, method: config.method }
  // 是否需要防止数据重复提交
  ...
  //保存到日志文件当中
  let data = JSON.stringify(myconfig, null, "\t");
  let name = 'request'+config.url+'.txt';//文件名
  // exportRaw(data, name);
  writeFile(name, data);
  return config
}, error => {
