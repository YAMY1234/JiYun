        ...
        <el-row>
          <el-col :span="24" v-if="form.parentId !== 0">
            <el-form-item label="上级目录" prop="parentId">
              <treeselect v-model="form.parentId" :options="mediaOptions" :normalizer="normalizer" placeholder="选择上级目录" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
<script>
import { listMedia, getMedia, delMedia, addMedia, updateMedia, listMediaExcludeChild } from "@/api/resource/media";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      let username = "李洋旻";
      listMedia(this.queryParams, username).then(response => {
        this.mediaList = this.handleTree(response.data, "mediaId");
        this.loading = false;
      });
    },
    /** 转换目录数据结构 */
    ...
    handleAdd(row) {
      this.reset();
      if (row != undefined) {
        this.form.parentId = row.mediaId;
      }
      this.open = true;
      this.title = "添加目录";
      listMedia().then(response => {
        this.mediaOptions = this.handleTree(response.data, "mediaId");
      });
    },
    /** 展开/折叠操作 */
    toggleExpandAll() {
    ...
    handleUpdate(row) {
      ...
      listMediaExcludeChild(row.mediaId).then(response => {
        this.mediaOptions = this.handleTree(response.data, "mediaId");
      });
    },
    /** 提交按钮 */
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.mediaId != undefined) {
